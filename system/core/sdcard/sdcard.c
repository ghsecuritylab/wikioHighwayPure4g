...
#define NO_STATUS 1
//Begin<Bug><JABAL-68><20150327>leave 100M for system&apk;xiongdajun
#define LIMIT_DATA_PARTITION_SIZE

#ifdef LIMIT_DATA_PARTITION_SIZE
static __u64 DATA_PARTITION_LIMITED_SIZE = 200 * 1024 * 1024;
#endif
//END<Bug><JABAL-68><20150327>leave 100M for system&apk;xiongdajun
...
//Begin<Bug><JABAL-68><20150327>leave 100M for system&apk;xiongdajun
    ...
    if (statfs(fuse->root.name, &stat) < 0) {
        ERROR("stat %s filesystem space failed \n",fuse->root.name);
        fuse->free_size =0;
    } else {
...
//Begin<Bug><JABAL-68><20150327>leave 100M for system&apk;xiongdajun
    ...
    if (!strncmp(fuse->root.name, "/data/media", fuse->root.namelen)) {
        ...
        pthread_mutex_unlock(&fuse->lock);
        if (fuse->free_size <= DATA_PARTITION_LIMITED_SIZE) {
            struct statfs stat;
            if (statfs(fuse->root.name, &stat) < 0) {
                ERROR("stat %s file system space failed\n",fuse->root.name);
                fuse->free_size =0;
                return -errno;
            } else {
                pthread_mutex_lock(&fuse->lock);
...
#ifdef LIMIT_DATA_PARTITION_SIZE
    if (!strncmp( fuse->root.name, "/data/media", fuse->root.namelen)) {
        out.st.blocks -= DATA_PARTITION_LIMITED_SIZE / out.st.bsize;
        if (out.st.bfree <= DATA_PARTITION_LIMITED_SIZE / out.st.bsize) {
            out.st.bfree = 0;
        } else {
            out.st.bfree -= DATA_PARTITION_LIMITED_SIZE / out.st.bsize;
        }

        if (out.st.bavail <= DATA_PARTITION_LIMITED_SIZE / out.st.bsize) {
            out.st.bavail = 0;
        } else {
            out.st.bavail -= DATA_PARTITION_LIMITED_SIZE / out.st.bsize;
        }
    }
#endif
//END<Bug><JABAL-68><20150327>leave 100M for system&apk;xiongdajun
