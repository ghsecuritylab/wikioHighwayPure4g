...
struct frame {
    const char *name;			//start add by lishengli 20140926     display num pic
    int disp_time;
    int min_capacity;
    bool level_only;
...
//start add by lishengli 20140926     display num pic
struct capacity_pic {
    const char *name;
    gr_surface surface;
};
...
struct capacityf{
    struct capacity_pic *pics;
    int num_pics;
    int y_offset;
};

static struct capacity_pic batt_cpct_surface[] = {
    {
        .name = "charger/cnum_0",
        .surface = NULL,
    },
    ...
    {
        .name = "charger/cpersent",
        .surface = NULL,
    }
};

static struct capacityf battery_capacity = {
    .pics = batt_cpct_surface,
    .num_pics = ARRAY_SIZE(batt_cpct_surface),
    .y_offset = 440,
};
//end add by lishengli 20140926    display num pic
...
struct charger {
    ...
    struct capacityf *batt_cpct;		//add by lishengli 20140926    display num pic
...
struct soc_led_color_mapping soc_leds[3] = {
    {15, RED_LED},
    {98, RED_LED | GREEN_LED},				//modified by kj 20150411 change capacity 99% to
    {100, GREEN_LED},
};
...
static int set_backlight(bool en)
{
    int fd;
    char buffer[10];
...
//start add by lishengli 20140926     display num pic
static int get_cap_data(int num, int *data)
{
    int i = 1;
...
static void draw_capaticy_pic(struct charger *charger)
{
    struct capacityf *cap_arr = charger->batt_cpct;
    int cap_num[5];
    int capnum_total;
    int cap_w = 0;
    int i = 0;
    ...
    memset(cap_num, 0, 5 * sizeof(int));
    capnum_total = get_cap_data(charger->batt_anim->capacity, cap_num);

    i = capnum_total;
    while(i >= 0)
    {
        cap_w += gr_get_width(cap_arr->pics[cap_num[i--]].surface);
    }
    cap_w += gr_get_width(cap_arr->pics[cap_arr->num_pics - 1].surface);

    x = (gr_fb_width() - cap_w) / 2 ;
    y = cap_arr->y_offset;
    i = capnum_total;
    while(i >= 0)
    {
        w = gr_get_width(cap_arr->pics[cap_num[i]].surface);
        h = gr_get_height(cap_arr->pics[cap_num[i]].surface);
		
        LOGV("drawing capacity surface %dx%d+%d+%d\n", w, h, x, y);
        gr_blit(cap_arr->pics[cap_num[i]].surface, 0, 0, w, h, x, y);
        x += w;
        i--;
    }
    w = gr_get_width(cap_arr->pics[cap_arr->num_pics - 1].surface);
    h = gr_get_height(cap_arr->pics[cap_arr->num_pics - 1].surface);
    gr_blit(cap_arr->pics[cap_arr->num_pics - 1].surface, 0, 0, w, h, x, y);
}
...
static void redraw_screen(struct charger *charger)
{
    ...
    if (batt_anim->capacity < 0 || batt_anim->num_frames == 0) {
        ...
        draw_capaticy_pic(charger);			//add by lishengli 20140926     display num pic
    }
    gr_flip();
}
...
{
    static int old_soc = -1;				//add by lishengli 20150410   fix JABALL-493    red led can't on when capacity is 0
    int soc;
    ...
    if (!charger->charger_connected) {
        ...
        if (charger->next_pwr_check == -1) {
            ...
            //charger->next_pwr_check = now + (UNPLUGGED_SHUTDOWN_TIME / 5);		//add by lishengli 20150514 lcd still on after charger plug out
            LOGI("[%" PRId64 "] device unplugged: shutting down in %" PRId64 " (@ %" PRId64 ")\n",
                 now, (int64_t)UNPLUGGED_SHUTDOWN_TIME, charger->next_pwr_check);

            if (!charger->batt_anim->run) {				//add by lishengli 20150414 can set led in sleep mode
                gr_fb_blank(false);
	         usleep(5000);
            }
            set_tricolor_led(0, GREEN_LED);		//add by lishengli 20150411 close led befor power off
            set_tricolor_led(0, RED_LED);
            ...
            LOGI("set_tricolor_led  run = %d \n", charger->batt_anim->run);
...
void healthd_mode_charger_init(struct healthd_config* /*config*/)
{
    ...
    if (mode == NORMAL) {
        ...
        if (!ret && !charging_enabled) {
            ...
            //LINE <JABALL-89> <DATA20150413> <power off charging> limi.zhang
            //android_reboot(ANDROID_RB_RESTART, 0, 0);
            android_reboot(ANDROID_RB_POWEROFF, 0, 0);
        }
    }
    ...
    charger->batt_cpct = &battery_capacity;		//add by lishengli 20140926    display num pic
    ...
    */
    ...
    for(i = 0; i < charger->batt_cpct->num_pics; i++)
    {
        struct capacity_pic *cpic = &charger->batt_cpct->pics[i];

        ret = res_create_display_surface(cpic->name, &cpic->surface);
        if (ret < 0) {
            LOGE("Cannot load capacity image %s\n", cpic->name);
            /* TODO: free the already allocated surfaces... */
            break;
        }
    }
