...
status_t CameraClient::startRecordingMode() {
    ...
    if(mPlayShutterSound)//songyutao add
...
void CameraClient::handleShutter(void) {
    if (mPlayShutterSound ) {
    	//ysg add for LJABALQJPZ-96
    	int ispano = mHardware->getParameters().getInt("burst-panorama");
    	 ALOGI("ysg ispano = %d" ,ispano );
    	if(ispano!=1)
...
void CameraClient::handleRawPicture(const sp<IMemory>& mem) {
    //disableMsgType(CAMERA_MSG_RAW_IMAGE);//zzp
