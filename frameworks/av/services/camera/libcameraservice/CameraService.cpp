...
int cpu_control  = 0; //cpu control flag
//char cpu_governor[4][20];
status_t CameraService::connect(
    ...
    String8 clientName8(clientPackageName);
    int callingPid = getCallingPid();
	//<BEGIN><JABALL-812><20150427>add for LCD flicker;xiongdajun
     int cpu_fd,i;
    char cpu_name[128];
	//<END><JABALL-812><20150427>add for LCD flicker;xiongdajun
    ALOGE("CameraService::connect E (pid %d \"%s\", id %d)", callingPid,
            clientName8.string(), cameraId);
	...
	for(i=0;i<4;i++)
	{
    		snprintf(cpu_name, sizeof(cpu_name), "/sys/module/lpm_levels/system/cpu%d/pc/idle_enabled", i);
		ALOGE("%s: dev name = %s, cam_idx = %d", __func__, cpu_name, i);
		cpu_fd = open(cpu_name, O_RDWR | O_NONBLOCK);
		if(cpu_fd>0)
		{
			ALOGE("%s: open %s OK\n", __func__,cpu_name);
			cpu_control = 1;
			ssize_t amt = write(cpu_fd, "0", 1);
			if (amt ==0)
				ALOGE("%s: write %s failed\n", __func__,cpu_name);
			else
				ALOGE("%s: write %s ok\n", __func__,cpu_name);
			close(cpu_fd);
		}
	...
	for(i=0;i<4;i++)
	{
    		snprintf(cpu_name, sizeof(cpu_name), "/sys/module/lpm_levels/system/cpu%d/standalone_pc/idle_enabled", i);
		ALOGE("%s: dev name = %s, cam_idx = %d", __func__, cpu_name, i);
		cpu_fd = open(cpu_name, O_RDWR | O_NONBLOCK);
		if(cpu_fd>0)
		{
	...
	for(i=0;i<4;i++)
	{
    		snprintf(cpu_name, sizeof(cpu_name), "/sys/devices/system/cpu/cpu%d/cpufreq/scaling_governor", i);
		ALOGE("%s: dev name = %s, cam_idx = %d", __func__, cpu_name, i);
		cpu_fd = open(cpu_name, O_RDWR | O_NONBLOCK);
		if(cpu_fd>0)
		{
			ALOGE("%s: open %s OK\n", __func__,cpu_name);
			cpu_control = 1;
		//	read(cpu_fd,&cpu_governor[i],20);

			ssize_t amt = write(cpu_fd, "performance", 11);
			if (amt ==0)
				ALOGE("%s: write %s failed\n", __func__,cpu_name);
			else
				ALOGE("%s: write %s ok\n", __func__,cpu_name);
	...
	if(cpu_control == 1)
	{
		...
		for(i=0;i<4;i++)
		{
			...
			if(cpu_fd>0)
			{
				...
				ssize_t amt = write(cpu_fd, "interactive", 11);
				if (amt ==0)
					ALOGE("%s: write %s failed\n", __func__,cpu_name);
				else
					ALOGE("%s: write %s ok\n", __func__,cpu_name);
