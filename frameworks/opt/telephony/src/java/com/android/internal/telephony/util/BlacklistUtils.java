...
public class BlacklistUtils {
	...
	//BEGIN<JABALL-786><20150407><8916L_SW2_V1.1:update black list>xusong
    public static boolean update(Context context, String number, int flags, int valid, long entryId) {
        ContentValues cv = new ContentValues();
        ...
        Uri uri = ContentUris.withAppendedId(Blacklist.CONTENT_URI, entryId);
        int count = context.getContentResolver().update(uri, cv, null, null);

        return count > 0;
    }
    public static boolean addOrUpdate(Context context, String number, int flags, int valid,long entryId) {

    	if (entryId == -1) {
    		return addOrUpdate(context, number, flags, valid);
    	} else {
    		return update(context, number, flags, valid, entryId);
    	}
    }

//END<JABALL-786><20150407><8916L_SW2_V1.1:update black list>xusong
    /**
