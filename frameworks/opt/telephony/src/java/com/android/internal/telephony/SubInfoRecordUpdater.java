...
public class SubInfoRecordUpdater extends Handler {
    private static final String LOG_TAG = "SubInfoRecordUpdater";
    ...
    private String mOperNameToSetSub0= null;//Line  to get the real SPN of Virtual Network Operator kj	
    private String mOperNameToSetSub1= null;//Line  to get the real SPN of Virtual Network Operator kj	
...
//End  to get the real SPN of Virtual Network Operator kj 			 
    ...
    private final BroadcastReceiver sReceiver = new  BroadcastReceiver() {
		...
		boolean mShowPlmn; 
                ...
                if (IccCardConstants.INTENT_VALUE_ICC_LOADED.equals(simStatus)) {
                    ...
                    if (SubscriptionManager.isValidSubId(subId)) {
                        ...
                        if (subInfo != null
                            ...
                            logd("Init mOperNameToSetSub0 = " + mOperNameToSetSub0);//Line  to get the real SPN of Virtual Network Operator kj		
                            logd("Init mOperNameToSetSub1 = " + mOperNameToSetSub1);//Line  to get the real SPN of Virtual Network Operator kj		
							
							
                            if((slotId == 0) && (mOperNameToSetSub0 != null)){
                                 nameToSet = mOperNameToSetSub0;
                            }else if((slotId == 1) && (mOperNameToSetSub1 != null)){
                                 nameToSet = mOperNameToSetSub1;
                            }else if(mSpnOverride.containsCarrier(CarrierName)) {
			...
			logd("SPN_STRINGS_UPDATED_ACTION slotId: " + slotId);
      
			logd("SPN_STRINGS_UPDATED_ACTION mSpn: " + mSpn);
			logd("SPN_STRINGS_UPDATED_ACTION mShowSpn: " + mShowSpn);
			logd("SPN_STRINGS_UPDATED_ACTION mPlmn: " + mPlmn);
			logd("SPN_STRINGS_UPDATED_ACTION mShowPlmn: " + mShowPlmn);			
			

			long subId = intent.getLongExtra(PhoneConstants.SUBSCRIPTION_KEY,
			        SubscriptionManager.INVALID_SUB_ID);
            
			logd("SPN_STRINGS_UPDATED_ACTION subId: " + subId);      
				...
				if(mSpn != null){
				   nameToSet = mSpn + " 0" + Integer.toString(slotId + 1);				
				}else if((mPlmn != null) && mShowPlmn && (!mShowSpn)){
				   nameToSet = mPlmn + " 0" + Integer.toString(slotId + 1);		                           
				}

				if((slotId == 0) && (nameToSet != null)){
					mOperNameToSetSub0 = nameToSet;
				}else if((slotId == 1) && (nameToSet != null)){
				       mOperNameToSetSub1 = nameToSet;
				}

				logd("SPN_STRINGS_UPDATED_ACTION  mSpn = " + mSpn);	
				logd("SPN_STRINGS_UPDATED_ACTION  mOperNameToSetSub0 = " + mOperNameToSetSub0);				
				logd("SPN_STRINGS_UPDATED_ACTION  mOperNameToSetSub1 = " + mOperNameToSetSub1);
