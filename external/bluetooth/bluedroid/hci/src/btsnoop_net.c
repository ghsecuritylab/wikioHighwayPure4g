...
static int local_socket_create(void) {
  ...
  if (listen(listen_socket_local_, 1) < 0) {
    ALOGE("Local socket listen failed (%s)", strerror(errno));
...
void btsnoop_net_write(const void *data, size_t length) {
  ...
  if (client_socket_btsnoop != -1) {
    do {
      ...
      if (ret < length) {
        ALOGE("%s: send : not able to write complete packet", __func__);
      }
      length -= ret;
...
static void *listen_fn_(UNUSED_ATTR void *context) {
  ...
  for (;;) {
    ...
    if(retval == 2) {
        ALOGI("%s sockets shutdown :", __func__);
        break;
    }
    ...
    if (FD_ISSET(listen_socket_, &sock_fds)) {
      ...
      if (client_socket == -1) {
        if (errno == EINVAL || errno == EBADF) {
          ALOGW("%s error accepting LOCAL socket: %s", __func__, strerror(errno));
          break;
        }
        ALOGW("%s error accepting LOCAL socket: %s", __func__, strerror(errno));
        continue;
