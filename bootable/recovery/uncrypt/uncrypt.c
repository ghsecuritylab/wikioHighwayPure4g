...
#define TINNO_MULTI_UPDATE_PACKAGE//lion add
...
char* fnArray[10];
char mapArray[10][1024];
int fnArryIndex = -1;
char* intToStr (int index){
	char* i = NULL;
	switch(index){
		case 0:i = "0";break;
		case 1:i = "1";break;
...
//lion add start
			...
			if (fn) {
				...
				ALOGE("step1.1 newline %p\n",newline);
				if (newline) *newline = 0;
			}
			fnArray[++fnArryIndex] = fn;
            strncpy(temp,"--update_package=@" CACHE_BLOCK_MAP , sizeof(temp));
	        ALOGE("temp =  %s\n", temp);
			strncat(temp, intToStr(fnArryIndex), sizeof(temp));
	        ALOGE("temp =  %s\n", temp);
			strncat(temp, "\n", sizeof(temp));
	        ALOGE("temp4 =  %d \n", sizeof(mapArray[0]));
			snprintf(mapArray[fnArryIndex],sizeof(mapArray[fnArryIndex]),"%s%s",CACHE_BLOCK_MAP,intToStr(fnArryIndex));
	        ALOGE("mapArray =  %s\n", mapArray[i++]);
#else
...
//lion add start
	...
	do{
		if(fnArryIndex >= 0 ){
			input_path = fnArray[fnArryIndex];
			map_file = mapArray[fnArryIndex];
			fnArryIndex--;
		}
#endif
//end
