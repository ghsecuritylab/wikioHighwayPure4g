...
#define GYRO_FILE_FOR_HAL_PATH  "/persist/gyro_cali_data"
/*****************************************************************************/
	  ...
	  cali_flag(0)
{
	...
	if(is_file(GYRO_FILE_FOR_HAL_PATH)&& (cali_flag == 0)){
		    ret=gyro_read_file();
		    if(ret ==0){
		    cali_flag=1;
		    ALOGE("gyro.x= %f, gyro.y=%f gyro.z=%f \n",cali[0],cali[1],cali[2]); 		 
		}		
	}
...
int GyroSensor::setInitialState() {
	...
	if (!ioctl(data_fd, EVIOCGABS(EVENT_TYPE_GYRO_X), &absinfo_x) &&
		...
		temp_data = value * CONVERT_GYRO_X;
		if(cali_flag==1) temp_data +=(cali[0]);				
		mPendingEvent.data[0] = temp_data;
		value = absinfo_y.value;
		temp_data = value * CONVERT_GYRO_Y;
		if(cali_flag==1) temp_data +=(cali[1]);	
		mPendingEvent.data[1] = temp_data;
		value = absinfo_z.value;
		temp_data = value * CONVERT_GYRO_Z;
		if(cali_flag==1) temp_data +=(cali[2]);
 ...
 int GyroSensor::gyro_read_file(){

             FILE *fp;

           fp= fopen(GYRO_FILE_FOR_HAL_PATH,"r");  
 
            if(fp == NULL)  
            {  
		    ALOGE("open %s error. \n",GYRO_FILE_FOR_HAL_PATH);  
		  return -1;
            }  
            else  
            {  
                  ...
                  fscanf(fp, "%f, %f, %f", &cali[0], &cali[1],&cali[2]);  
		    ALOGE("gyro->x =%f,gyro->y=%f,gyro->z=%f\n",cali[0],cali[1],cali[2]);		  
                  fclose(fp);  
            	}
...
bool GyroSensor:: is_file(const char *path)
{
    struct stat statbuf;
...
again:
	...
	while (count && mInputReader.readEvent(&event)) {
		...
		if (type == EV_ABS) {
			...
			//ALOGE("GyroSensor: Original: code = %d; value = %f ", event->code, value);			
			
			if (event->code == EVENT_TYPE_GYRO_X) {
				temp_data = value * CONVERT_GYRO_X;
				//ALOGE("GyroSensor: Before: code = %d; value = %f ", event->code, temp_data);			
				if(cali_flag==1) temp_data +=(cali[0]);				
				mPendingEvent.data[0] = temp_data;
			...
			//ALOGE("GyroSensor: After: code = %d; value = %f ", event->code, temp_data);			
		} else if (type == EV_SYN) {
			switch ( event->code ){
				case SYN_TIME_SEC:
					{
