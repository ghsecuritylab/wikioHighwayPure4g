...
//yangliang add to cancle echo in use-case of hangouts handset-mic 20150525
void platform_set_echo_reference_tinno_hangouts_hsmic(void *platform, bool enable)
{
    struct platform_data *my_data = (struct platform_data *)platform;
    struct audio_device *adev = my_data->adev;

    ALOGV("platform_set_echo_reference_tinno_hangouts_hsmic: device %d", adev->out_device);
	
    if (enable) {
        my_data->ec_ref_enabled = enable;
	    ALOGV("platform_set_echo_reference_tinno_hangouts_hsmic: enable Audio EC\n");
        audio_route_apply_and_update_path(adev->audio_route, "echo-reference-tinno-hangouts");
    } else {
        if (my_data->ec_ref_enabled) {
	        ALOGV("platform_set_echo_reference_tinno_hangouts_hsmic: Disable Audio EC\n");
            audio_route_reset_and_update_path(adev->audio_route, "echo-reference-tinno-hangouts");
            my_data->ec_ref_enabled = enable;
        } else {
            ALOGV("platform_set_echo_reference_tinno_hangouts_hsmic: EC reference is already disabled : %d", my_data->ec_ref_enabled);
        }
    }

    ALOGV("platform_set_echo_reference_tinno_hangouts_hsmic: Setting EC Reference: %d", enable);
}
...
void platform_add_backend_name(char *mixer_path, snd_device_t snd_device)
{
    ...
    //<BEGIN><FFBAKK-105><2015.02.02>add for tfa9890-pa-dsp mode 4-lines;yangliang
    #if defined(TARGET_L9010)
    else if (snd_device == SND_DEVICE_OUT_SPEAKER || snd_device == SND_DEVICE_OUT_VOICE_SPEAKER)
...
snd_device_t platform_get_input_snd_device(void *platform, audio_devices_t out_device)
{
    ...
    if ((out_device != AUDIO_DEVICE_NONE) && ((mode == AUDIO_MODE_IN_CALL) ||
        ...
        if (adev->active_input) {
			...
			if(1){//yangliang add to cancle echo in use-case of hangouts handset-mic 20150525
				ALOGV("Disable echo reference for handset-mic //test only\n");
				platform_set_echo_reference(adev->platform, false);
			}
                } else if (in_device & AUDIO_DEVICE_IN_WIRED_HEADSET) {
                    snd_device = SND_DEVICE_IN_HEADSET_MIC_FLUENCE;
                }
		   if(out_device == 0x2/*speaker*/){//yangliang add to cancle echo in use-case of hangouts handset-mic 20150525
		         ALOGV("Set echo reference for speaker\n");
                 platform_set_echo_reference(adev->platform, true);
		   }
		   else{//yangliang add to cancle echo in use-case of hangouts handset-mic 20150525
		         ALOGV("Set echo reference for not-speaker\n");
			     platform_set_echo_reference_tinno_hangouts_hsmic(adev->platform, true);
		   }
    ...
    if (in_device != AUDIO_DEVICE_NONE &&
        ...
        if (in_device & AUDIO_DEVICE_IN_BUILTIN_MIC) {
            ...
            else if (my_data->fluence_type & FLUENCE_DUAL_MIC && channel_count == 2)//<BEGIN><FFBAKK-105><2014.01.04>add for double mic;yangliang
            	snd_device = SND_DEVICE_IN_HANDSET_STEREO_DMIC;
            else
                snd_device = SND_DEVICE_IN_HANDSET_MIC;
