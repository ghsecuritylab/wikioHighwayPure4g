...
//<BEGIN><FFBAKK-105><2014.01.12>add for tfa9890-pa;yangliang
                       ...
                       struct audio_usecase *usecase)
{
    ...
    //<BEGIN><FFBAKK-105><2015.02.02>add for tfa9890-pa-dsp mode;yangliang
	#if defined(TARGET_L9010) 
	/*yangliang Add NXP external PA on function here*/
/*    if (snd_device == SND_DEVICE_OUT_SPEAKER ||
        snd_device == SND_DEVICE_OUT_SPEAKER_AND_HEADPHONES ||
        snd_device == SND_DEVICE_OUT_VOICE_SPEAKER) {	*/
	if (snd_device == SND_DEVICE_OUT_SPEAKER || snd_device == SND_DEVICE_OUT_SPEAKER_REVERSE \
		|| snd_device == SND_DEVICE_OUT_VOICE_SPEAKER || snd_device == SND_DEVICE_OUT_SPEAKER_AND_HDMI \
		|| snd_device == SND_DEVICE_OUT_SPEAKER_AND_USB_HEADSET || snd_device == SND_DEVICE_OUT_SPEAKER_AND_ANC_HEADSET \
		|| snd_device == SND_DEVICE_OUT_SPEAKER_PROTECTED \
		|| snd_device == SND_DEVICE_OUT_SPEAKER_AND_HEADPHONES) {        
		if ((adev->snd_dev_ref_cnt[snd_device] == 1) || (adev->snd_dev_ref_cnt[snd_device] == 2)) {//yangliang add 2 for one-tx voice-and-rx meanwhile with playmusic background and then spk-nosound;0709
		ALOGE("%s: enable out--speaker-of-pa-3", __func__);
		if((!tfa9890_check_tfaopen())) {
			ALOGE("%s: enable out--speaker-of-pa-4 (g_handle == -1)", __func__);
			tfa9890_init();
		}
		else{
			ALOGE("%s: enable out--speaker-of-pa-6 (g_handle != -1)", __func__);
		}
		ALOGE("%s: enable out--speaker-of-pa-5", __func__);

		if (strcmp(mixer_path,"voice-call speaker")) {//yangliang add for diff EQ-mode0423
			if (strcmp(mixer_path,"low-latency-playback speaker")){
				ALOGE("%s: yangliang not go into voice-call speaker and not low-latency-playback speaker", __func__);
				tfa9890_setSamplerate(48000);
				tfa9890_EQset(0);
				tfa9890_SpeakerOn();
		 	} else{
				ALOGE("%s: yangliang not go into voice-call speaker and is low-latency-playback speaker", __func__);
				tfa9890_setSamplerate(48000);
				tfa9890_EQset(2);
				tfa9890_SpeakerOn();		 
		 	}
		...
		if ( !strcmp(mixer_path,"voice-call speaker")){//voice-call-spk,yangliang add for diff EQ-mode0423
			ALOGE("%s: yangliang go into voice-call speaker", __func__);
			tfa9890_setSamplerate(48000);
			tfa9890_EQset(2);
			tfa9890_SpeakerOn();
		} 
                        ...
                        struct audio_usecase *usecase)
{
    ...
    //<BEGIN><FFBAKK-105><2015.02.02>add for tfa9890-pa-dsp mode;yangliang
    #if defined(TARGET_L9010) 
    /*yangliang Add NXP external PA off function here*/
    /*    if (snd_device == SND_DEVICE_OUT_SPEAKER ||
        snd_device == SND_DEVICE_OUT_SPEAKER_AND_HEADPHONES ||
        snd_device == SND_DEVICE_OUT_VOICE_SPEAKER) {	*/
    if (snd_device == SND_DEVICE_OUT_SPEAKER || snd_device == SND_DEVICE_OUT_SPEAKER_REVERSE \
       ...
       if (adev->snd_dev_ref_cnt[snd_device] == 1) {
          ALOGE("%s: disable out--speaker-of-pa-1", __func__);
	  tfa9890_SpeakerOff();
	}
    }
    #endif	
                                   ...
                                   uint32_t *dsp_frames)
{
    ...
    //BEGIN<><20150615><CR:828864 PDF20150610>wangyanhui
     ...
     *dsp_frames = 0;
     if (is_offload_usecase(out->usecase)) {
     //END<><20150615><CR:828864>wangyanhui
        ssize_t ret = 0;
        //*dsp_frames = 0;//LINE<><20150615><CR:828864>wangyanhui
        pthread_mutex_lock(&out->lock);
        if (out->compr != NULL) {
    ...
    //BEGIN<><20150615><CR:828864>wangyanhui
